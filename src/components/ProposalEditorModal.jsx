...
import { 
    ...
    getAllPunteggi, 
    getMattoniAura, 
    getClassiOggetto // Assicurati che questa funzione esista in api.js
} from '../api';
...
const ProposalEditorModal = ({ proposal, type, onClose, onRefresh }) => {
    ...
    // --- STATO FILTRO CLASSE ---
    const [selectedClasseId, setSelectedClasseId] = useState(''); // ID della classe selezionata per il filtro
    
    // Mappa componenti { id_caratteristica: valore }
    const initialComponents = {};
    if (proposal?.componenti) {
        proposal.componenti.forEach(c => {
            // Supporto robusto per formati diversi (oggetto o ID)
    ...
    const [availableMattoni, setAvailableMattoni] = useState([]); // Mattoni specifici dell'aura scelta
    const [availableClassi, setAvailableClassi] = useState([]);   // Lista classi oggetto
    ...
    // --- EFFETTO 1: CARICAMENTO INIZIALE ---
    useEffect(() => {
        const initData = async () => {
            setIsLoadingData(true);
            try {
                // 1. Carica tutti i punteggi
                ...
                if (char && char.punteggi_base) {
                    ...
                    const validAuras = allData.filter(p => {
                        ...
                        if (!val || val < 1) return false; // Deve possederla
                ...
                // 2. Carica Classi Oggetto (Solo se è una Infusione)
                if (isInfusion) {
    ...
    // --- EFFETTO 3: LOGICA AURE SECONDARIE ---
    useEffect(() => {
    ...
    // --- FUNZIONE DI FILTRO COMPATIBILITÀ ---
    // Determina se una caratteristica è compatibile con la classe selezionata
    const isCharCompatibleWithClass = (charId) => {
        ...
        // Determina tipo Aura (Tecnologica vs Mondana)
        // Usiamo una logica euristica sul nome se non c'è un flag specifico
        const auraObj = availableAuras.find(a => a.id == selectedAuraId);
        const isTecno = auraObj?.nome?.toLowerCase().includes("tecnologic"); 
        
        // Controlla le liste di permessi della classe
        if (isTecno) {
    ...
    const getVisualInfo = (charObj) => {
        ...
        // Per la secondaria semplifichiamo (non carichiamo mattoni secondari qui per ora)
        const secondaryBrick = null; 
        ...
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-2 sm:p-4">
            <div className="bg-gray-900 w-full max-w-4xl max-h-[95vh] rounded-xl flex flex-col border border-gray-700 shadow-2xl animate-fadeIn">
                    ...
                    {isInfusion && selectedAuraId && (
                        <div className="bg-indigo-900/10 border border-indigo-500/30 p-3 rounded-lg flex flex-col sm:flex-row gap-4 items-center">
                            <div className="flex-1 w-full">
                                <label className="text-xs font-bold text-indigo-300 uppercase flex items-center gap-2 mb-1">
                                    <Box size={14}/> Progetta per Classe Oggetto
                                </label>
                                <select 
                                    className="w-full bg-gray-900 border border-indigo-500/50 rounded p-2 text-white text-sm"
                                    value={selectedClasseId}
                                    onChange={e => setSelectedClasseId(e.target.value)}
                                >
                                    <option value="">-- Nessun filtro (Mostra tutto) --</option>
                                    {availableClassi.map(cls => (
                                        <option key={cls.id} value={cls.id}>
                                            {cls.nome} (Evidenzia compatibili)
                                        </option>
                                    ))}
                                </select>
                            </div>
                            <div className="text-xs text-indigo-400/80 w-full sm:w-auto sm:max-w-xs">
                                <Info size={12} className="inline mr-1"/>
                                Seleziona una classe per oscurare i componenti non compatibili con essa.
                            </div>
                        </div>
                    )}
                    ...
                    <div className="bg-gray-800/50 rounded border border-gray-700 p-2">
                        ...
                        <div className="space-y-2 max-h-[300px] overflow-y-auto pr-2 custom-scrollbar">
                            {availableCharacteristics.map(charObj => {
                                ...
                                const opacityClass = isCompatible ? 'opacity-100' : 'opacity-40 grayscale';
                                const bgClass = count > 0 ? 'bg-indigo-900/20 border-indigo-500' : 'bg-gray-800 border-gray-700';

                                return (
                                    <div key={charObj.id} className={`flex items-center justify-between p-2 rounded border transition-all ${bgClass} ${opacityClass}`}>
                                        <div className="flex items-center gap-3">
                                            ...
                                            <div className="flex items-center gap-2">
                                                ...
                                                {hasPrimary && (
                                                    <div className="flex -space-x-1">
                                                        <div className="relative z-10" title={`Mattone: ${primaryBrick.nome}`}>
                                                            <IconaPunteggio url={primaryBrick.icona_url} color={primaryBrick.colore} size="xs" />
                                                        </div>
                                                        {hasSecondary && (
                                                            <div className="relative z-0" title={`Infusione: ${secondaryBrick.nome}`}>
                                            ...
                                            <div>
                                                ...
                                                {!isCompatible && (
                                                    <div className="text-[10px] text-red-400 font-bold uppercase tracking-wider mt-0.5 flex items-center gap-1">
                                                        <AlertTriangle size={10}/> Non Compatibile
                                                    </div>
                                                )}
                                            </div>
                                        </div>
                                        ...
                                        <div className="flex items-center gap-2 bg-gray-900 p-1 rounded border border-gray-700">
                                            <button onClick={() => handleDecrement(charObj.id)} disabled={!isDraft || count === 0} className="w-6 h-6 flex items-center justify-center text-gray-400 hover:text-white hover:bg-gray-800 rounded"><Minus size={12}/></button>
                                            <span className="w-6 text-center text-white font-mono">{count}</span>
                                            {/* Nota: Permettiamo l'incremento anche se incompatibile (a rischio utente), o blocchiamo? Qui blocco se non compatibile */}
                                            <button onClick={() => handleIncrement(charObj.id)} disabled={!isDraft || !isCompatible} className="w-6 h-6 flex items-center justify-center text-gray-400 hover:text-white hover:bg-gray-800 rounded disabled:opacity-30"><Plus size={12}/></button>
                                        </div>
                                    </div>
                                );
                            })}
                ...
                <div className="p-4 border-t border-gray-700 bg-gray-800 rounded-b-xl flex justify-between shrink-0">
                    {isDraft && isEditing ? <button onClick={handleDelete} className="text-red-400 text-xs flex items-center gap-1 hover:text-red-300 transition-colors"><Trash2 size={14}/> Elimina Bozza</button> : <div/>}
                    <div className="flex gap-2">
                        {isDraft && (
                            <>
                                <button onClick={() => handleSaveAction(false)} disabled={isSaving} className="px-3 py-2 bg-gray-700 hover:bg-gray-600 text-white rounded text-sm flex gap-2 items-center transition-colors"><Save size={14}/> Salva</button>
                                <button onClick={() => handleSaveAction(true)} disabled={isSaving || currentTotalCount === 0} className="px-3 py-2 bg-green-600 hover:bg-green-500 text-white rounded text-sm flex gap-2 items-center font-bold shadow-lg shadow-green-900/20 transition-colors"><Send size={14}/> Invia</button>
                            </>
                        )}
                    </div>
                </div>
