...
const TecnicaDetailModal = ({ tecnica, onClose, type = 'tecnica' }) => {
  ...
  // Scegliamo quale testo mostrare: 
  // Se posseduta (testo_formattato_personaggio) ha precedenza perché ha i valori calcolati del PG.
  // Nota: Il backend ora include già la Formula in calce a questo testo.
  const testoDescrizione = tecnica.testo_formattato_personaggio || tecnica.TestoFormattato || tecnica.testo;

  // Calcolo costo stimato se non fornito dal backend (fallback)
  const costoMattoni = tecnica.costo_crediti || (tecnica.livello * 100);
  ...
  return (
    ...
    >
      <div 
        className="relative w-full max-w-lg p-6 bg-gray-800 rounded-xl shadow-2xl border border-gray-700 max-h-[90vh] overflow-y-auto"
      ...
      >
        ...
        {/* Corpo del testo formattato (Include Formula) */}
        ...
        <div className="mt-2 pt-4 border-t border-gray-700 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2 text-xs text-gray-400 font-mono">
            <span className="bg-gray-900 px-2 py-1 rounded">
                Costo: <span className="text-yellow-500 font-bold">{costoMattoni} CR</span>
            </span>
