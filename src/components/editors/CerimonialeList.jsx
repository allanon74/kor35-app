...
const CerimonialeList = ({ onAdd, onEdit, onLogout }) => {
  ...
  const loadData = () => staffGetCerimoniali(onLogout).then(setItems);
  useEffect(() => { loadData(); }, []);
    ...
    <div className="space-y-4">
      <div className="flex justify-between items-center bg-gray-800 p-4 rounded-xl border border-gray-700 shadow-lg">
        <div className="flex items-center gap-4">
          ...
          <input type="text" placeholder="Cerca..." className="bg-gray-950 border border-gray-700 rounded-lg px-4 py-2 text-sm focus:border-amber-500 outline-none w-64" value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        </div>
        <button onClick={onAdd} className="bg-amber-600 hover:bg-amber-500 px-6 py-2 rounded-lg font-black text-sm transition-all shadow-md uppercase">+ Nuovo Cerimoniale</button>
      </div>
