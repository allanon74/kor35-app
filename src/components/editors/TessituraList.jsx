...
const TessituraList = ({ onAdd, onEdit, onLogout }) => {
  ...
  const loadData = () => staffGetTessiture(onLogout).then(setItems);
  useEffect(() => { loadData(); }, []);

  const handleDelete = async (id) => {
    if (window.confirm("Eliminare questa Tessitura?")) {
    ...
    <div className="space-y-4">
      <div className="flex justify-between items-center bg-gray-800 p-4 rounded-xl border border-gray-700 shadow-lg">
        <div className="flex items-center gap-4">
          ...
          <input type="text" placeholder="Cerca..." className="bg-gray-950 border border-gray-700 rounded-lg px-4 py-2 text-sm focus:border-cyan-500 outline-none w-64" value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        </div>
        <button onClick={onAdd} className="bg-cyan-600 hover:bg-cyan-500 px-6 py-2 rounded-lg font-black text-sm transition-all shadow-md uppercase">+ Nuova Tessitura</button>
      </div>
          ...
          <tbody className="divide-y divide-gray-700/50 text-sm">
            {filteredItems.map(item => (
              <tr key={item.id} className="hover:bg-gray-700/30 transition-colors group text-left">
                ...
                <td className="px-6 py-4"><span className="bg-cyan-900 text-cyan-400 px-2 py-1 rounded text-xs font-bold">Lvl {item.livello}</span></td>
                <td className="px-6 py-4 text-xs font-bold uppercase text-indigo-400">{item.aura_richiesta?.nome || '---'}</td>
                <td className="px-6 py-4 text-xs font-bold uppercase text-emerald-400">{item.elemento_principale?.nome || '---'}</td>
